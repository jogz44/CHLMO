<div class="app-page">
    ...
    <div class="grid lg:grid-cols-4 md:grid-cols-4 grid-row-1 lg:gap-10 md:gap-10 gap-1 mb-8 mx-2 lg:mx-0 md:mx-0">
        ...
        <a href="{{ route('transaction-request') }}">
            <!-- Card 3 -->
            <div @click="window.location.href='{{ route('transaction-request') }}'"
                class="relative cursor-pointer bg-white shadow rounded-lg flex items-center">
                <div class="flex-shrink-0 mr-4">
                    <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="50" height="50" fill="currentColor" stroke="currentColor" viewBox="0 0 48 48" class="w-6 h-6 ml-5 text-custom-yellow" stroke-width="0.5">
        ...
        <a href="{{ route('awardee-list') }}">
            <div @click="window.location.href='{{ route('awardee-list') }}'"
                class="relative cursor-pointer bg-white shadow rounded-lg flex items-center">
                <div class="flex-shrink-0 mr-4">
                    <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="50" height="50" fill="currentColor" stroke="currentColor" viewBox="0 0 48 48" class="w-6 h-6 ml-5 text-custom-yellow" stroke-width="0.5">
        ...
        <a href="{{ route('blacklist') }}">
            <!-- Card 4 -->
            <div @click="window.location.href='{{ route('blacklist') }}'"
                class="relative cursor-pointer bg-white shadow rounded-lg flex items-center">
                <div class="flex-shrink-0 mr-4">
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            ...
            const palette = [{
                    border: 'rgba(255, 145, 0, 1)',
                    fill: 'rgba(255, 145, 0, 0.30)'
                }, // orange
                {
                    border: 'rgb(0, 23, 173)',
                    fill: 'rgba(0, 23, 173, 0.30)'
                }, // blue
                {
                    border: 'rgb(0, 144, 29)',
                    fill: 'rgba(0, 144, 29, 0.30)'
                }, // green
                {
                    border: 'rgba(150, 90, 200, 1)',
                    fill: 'rgba(150, 90, 200, 0.30)'
                }, // purple
            ];
            ...
            function buildRelocationDatasets(data) {
                return Object.keys(data.datasets).map((type, i) => {
            ...
            function initRelocationChart(data) {
                const ctx1 = document.getElementById('relocationLotChart').getContext('2d');
                relocationChart = new Chart(ctx1, {
                    type: 'line',
                    data: {
                        labels: data.labels,
                        datasets: buildRelocationDatasets(data)
                    },
                    options: {
                        responsive: true,
                        plugins: {
            ...
            function initInformalChart(data) {
                const ctx2 = document.getElementById('informalSettlersChart').getContext('2d');
                informalChart = new Chart(ctx2, {
                    type: 'bar',
                    data: {
                        labels: data.labels,
                        datasets: [{
                            label: 'Applicants',
                            data: data.informalSettlers,
                            backgroundColor: (context) => {
                                ...
                                gradient.addColorStop(0, '#0071AD');
                                gradient.addColorStop(1, '#3FA9E0');
                    ...
                    options: {
                        indexAxis: 'y', // horizontal bars — reads better with long labels
                        responsive: true,
                        plugins: {
                            legend: {
                                display: false
            ...
            // Initial render (data already available server-side on first load)
            const initialRelocationData = @js($relocationLotData);
            const initialInformalData = @js($informalSettlersData);

            initRelocationChart(initialRelocationData);
            initInformalChart(initialInformalData);

            // Redraw charts whenever the year filter changes
            Livewire.on('charts-updated', ({
                relocationLotData,
                informalSettlersData
            }) => {
                relocationChart.data.labels = relocationLotData.labels;
                relocationChart.data.datasets = buildRelocationDatasets(relocationLotData);
                relocationChart.update();

                informalChart.data.labels = informalSettlersData.labels;
                informalChart.data.datasets[0].data = informalSettlersData.informalSettlers;
                informalChart.update();
            });
        });
    </script>
    @endpush
